...
const BADGES = {
  NONE: '', ZOOMED: '←  →', SPEED: '▶️', REFRESH: '↺', WARNING: '!',
};
...
const STATE_DATA: BackgroundStateMap = {
  ...
  UNZOOMED: {
    badge: BADGES.NONE, title: '', // if empty, reloaded from manifest
    showpopup: false, zoomed: false, color: DEFAULT_COLOR,
  }, ZOOMING: {
    badge: BADGES.ZOOMED, title: 'Searching for videos to zoom', showpopup: false, zoomed: true, color: DEFAULT_COLOR,
  }, ZOOMING_SPEED_ONLY: {
    badge: BADGES.ZOOMED, title: 'Searching for videos enhance', showpopup: true, zoomed: true, color: DEFAULT_COLOR,
  }, ZOOMED_NOSPEED: {
    badge: BADGES.SPEED,
    ...
    badge: BADGES.SPEED, title: 'Click to change speed or unzoom', showpopup: true, zoomed: true, color: DEFAULT_COLOR,
  }, SPEED_ONLY: {
    badge: BADGES.SPEED, title: 'Click to change speed', showpopup: true, zoomed: true, color: DEFAULT_COLOR,
  }, REFRESH: {
    ...
    badge: BADGES.WARNING, title: 'Permission denied by user', showpopup: false, zoomed: false, color: '#FCD2D2F7',
  }, ERR_URL: {
    ...
    badge: BADGES.SPEED, title: '', showpopup: false, zoomed: false, color: DEFAULT_COLOR,
  },
};
...
function setSubframeData(tabId: number, domain: string, subFrameStr: string) {
  ...
  g_globalAccessSubframeData[tabId] = {
    tabId, domain, subFrameStr,
  };
}

async function setSpeedGlobalData(tabId: number, domain: string, speed: string): Promise<boolean> {
  try {
...
async function getGlobalSpeedData(tabId: number, domain: string): Promise<string> {
  try {
    const key = `speed.${tabId}.${domain}`;
    const orgValue = await chrome.storage.session.get(key);
...
async function setLastUrlTitleFromOnUpdated(tabId: number, url: string, title: string): Promise<void> {
  try {
...
async function setCurrentTabState(tabId: number, startingState: BackgroundState, domain: string = '', speed: string = DEAULT_SPEED): Promise<void> {
  try {
    ...
    const popup = showpopup ? `popup.html#tabId=${tabId}&speed=${speed}&domain=${domain}&badge=${badge}` : '';
...
async function getCurrentTabState(tabId: number): Promise<BackgroundState> {
  try {
    ...
    const keys: BackgroundState[] = backgroundStates.filter(k => STATE_DATA[k].title === title);
    if (!keys?.length) {
 ...
 * @returns {boolean}
 */
const injectionResultCheckBool = (injectionResults: InjectionResult<boolean>[], defaultVal: boolean = false): boolean => {
...
async function DoInjectZoomJS(tabId: number): Promise<void> {
  try {
    ...
    await chrome.scripting.executeScript({
                                           target: {
                                             tabId, allFrames: true,  // false doesn't hide some content.
                                           }, // world:  "MAIN",
...
async function DoInjectZoomCSS(tabId: number, isDummy: boolean = false, // for JS only injection (no zoom), we
  ...
  try {
    ...
    await chrome.scripting.executeScript({
                                           ...
                                           func: injectCssHeader, args: [cssFilePath, CSS_STYLE_HEADER_ID],
                                         });
...
async function DoUndoInjectCSS(tabId: number): Promise<void> {
  try {
    ...
    await chrome.scripting.executeScript({
                                           ...
                                           func: uninjectCssHeader, args: [CSS_STYLE_HEADER_ID],
                                         });
...
async function unZoom(tabId: number, domain: string): Promise<void> {
  try {
    trace('unZoom enter');
    await Promise.all([setCurrentTabState(tabId, 'UNZOOMED'), DoUndoInjectCSS(tabId), setSpeed(tabId, domain, DEAULT_SPEED, false), chrome.scripting.executeScript({
                                                                                                                                                                     target: {
                                                                                                                                                                       tabId,
                                                                                                                                                                       allFrames: true,
                                                                                                                                                                     }, // world:  "MAIN",
...
async function DoZoom(tabId: number, state: BackgroundState, domain: string): Promise<void> {
  trace('DoZoom enter');
  try {
    ...
    if (excluded_zoom || state === 'SPEED_ONLY') {
      await Promise.all([DoInjectTagOnlyJS(tabId), // DoInjectZoomCSS(tabId, true),
                         setCurrentTabState(tabId, 'ZOOMING_SPEED_ONLY', domain)]);
    } else {
      await Promise.all([DoInjectZoomJS(tabId), DoInjectZoomCSS(tabId), setCurrentTabState(tabId, 'ZOOMING', domain)]);
...
function processIFrameExtraPermissionsResult(results: InjectionResult<string[]>[], tabId: number, domain: string): boolean {

  if (!GET_IFRAME_PERMISSIONS || results.length === 0) {
    return false;
  }
  const extraDomainsArry = results.map(o => o.result).flat().filter(str => str?.length > 0);
...
async function setSpeed(tabId: number, domain: string, speedStr: string = DEAULT_SPEED, allowPlaybackToggle: boolean = true): Promise<boolean> {
  try {
    ...
    if (!wasSetBefore && speedStr === DEAULT_SPEED) {
      trace('setSpeed: NOT setting video speed since it doesn\'t seem required (max compatability mode)');
      return false;
    }
...
async function getSpeed(tabId: number, domain: string, defaultSpeed: string = DEFAULT_SPEED): Promise<string> {
  try {
    ...
    const results = await chrome.scripting.executeScript({
                                                           ...
                                                           func: injectGetPlaypackSpeed, args: [],
                                                         });
...
async function skipPlayback(tabId: number, secondToSkipStr: string, domain: string): Promise<void> {
  try {
    trace('skipPlayback', tabId, secondToSkipStr);
    if (domain?.length && BLOCKED_SKIPFEATURE_DOMAINS.filter((d) => domain.includes(d)).length > 0) {
    ...
    await chrome.scripting.executeScript({
                                           ...
                                           func: injectVideoSkip, args: [secondToSkipStr],
                                         });
  } catch (err) {
    logerr(err);
  }
...
async function showUpgradePageIfNeeded(): Promise<void> {
  try {
    ...
    await chrome.tabs.create({
                               url: chrome?.runtime?.getURL('help.html'), active: false,
                             });
  } catch (err) {
    logerr(err);
  }
...
async function toggleZoomState(tabId: number, domain: string): Promise<boolean> {
...
chrome.action.onClicked.addListener((tab) => {
  ...
  try {
    ...
    if (!(tab?.url?.startsWith('https://') || tab?.url?.startsWith('http://') || tab?.url?.startsWith('file:'))) {
    ...
    chrome.storage.local.get((resultSettings) => {
      ...
      if (settings.allSitesAccess) {
        ...
        if (GET_IFRAME_PERMISSIONS && subFrameData !== false) {
          const iframeDomains = subFrameData.subFrameStr.split(',').map(d => domainToSiteWildcard(d, settings.wholeDomainAccess));
          origins.push(...iframeDomains);
          trace('Requesting extra iframe domains that blocked speedup', iframeDomains);
        }
      }
      ...
      chrome.permissions.request({
        ...
        if (!granted) {
          ...
          if (!fileAccessEnabledForExtention) {
            await chrome.tabs.create({
                                       url: chrome?.runtime?.getURL('help.html#localfile'), active: false,
                                     });
          }
          return;
        }
 ...
 */
const ReZoom = async (tabId: number, domain: string, speed: string): Promise<void> => {
  ...
  if (currentState === 'ZOOMING_SPEED_ONLY') {
    ...
    await Promise.all([setCurrentTabState(tabId, nextState, currentSpeed), DoZoom(tabId, currentState, currentSpeed), setSpeed(tabId, domain, currentSpeed, false)]);
  ...
  setTimeout(async () => {
    try {
      switch (cmd) {
        ...
        case 'UNZOOM_CMD':
          ...
          if (IS_BETA_CHANNEL) {
            ...
            if (!settings.beta3EndingShown) {
              await chrome.tabs.create({
                                         url: chrome?.runtime?.getURL('beta_ending.html'), active: false,
                                       });
        ...
        case 'SET_SPEED_CMD':
          await Promise.all([setCurrentTabState(tabId, 'ZOOMED_SPEED', domain, speed), setSpeed(tabId, domain, speed, true)]);
          break;
...
${JSON.stringify(tab, null, 2)}`);
    if (tabId && ['loading', 'completed'].includes(changeInfo?.status || '')) {
      ...
      setTimeout(async () => {  // async calls needed
...
chrome.runtime.onConnect.addListener((externalPort) => {
  ...
  }
  {
    ...
    if (!g_PopupOpenedForTabs.includes(tabId)) {
      ...
      trace(`Popup opened. Added tabId:'${tabId}' g_PopupOpenedForTabs: [${g_PopupOpenedForTabs.join(',')}]`);
    }
  }
  ...
  externalPort.onDisconnect.addListener((portDisconnectEvent) => {
    ...
    if (g_PopupOpenedForTabs.includes(tabId)) {
      trace(`Popup closed. Removed tabId:'${tabId}' g_PopupOpenedForTabs: [${g_PopupOpenedForTabs.join(',')}]`);
